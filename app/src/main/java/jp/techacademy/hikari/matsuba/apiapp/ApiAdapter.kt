...
import jp.techacademy.hikari.matsuba.apiapp.databinding.RecyclerFavoriteBinding
...
class ApiAdapter : ListAdapter<Shop, ApiItemViewHolder>(ApiItemCallback()) {
    ...
    //Shop…データを保持するクラス、ApiItemViewHolder…RecyclerViewで表示させる1行の内容を保持するクラスを指定
    //ApiResponse.ktにてval shop: List<Shop>と定義している。
    //ApiItemCallback() …「データの差分を確認するオブジェクト」を指定
...
class ApiItemViewHolder(private val binding: RecyclerFavoriteBinding) :
    ...
    fun bind(shop: Shop, position: Int,adapter: ApiAdapter) {
        ...
        // addressTextView（recycler_favorite.xmlで定義）のtextプロパティに代入されたオブジェクトのnameプロパティを代入

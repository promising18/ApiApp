...
import androidx.fragment.app.Fragment//Fragmentクラスを継承
...
class ApiFragment : Fragment() {
    ...
    private fun updateData() {
        ...
        val request = Request.Builder()
            .url(url)
            .build()
        client.newCall(request).enqueue(object : Callback { //ここで、実際にHttp通信を行う
            override fun onFailure(call: Call, e: IOException) { // Error時の処理
                e.printStackTrace()
                handler.post {
                    updateRecyclerView(listOf()) //RecyclerViewを空のリストで更新
                }
            }
            ...
            override fun onResponse(call: Call, response: Response) { // 成功時の処理
                ...
                //データをViewに反映させて描画させるのはメインメソッドとは別で処理をする必要がある。
            }
        })
    }
